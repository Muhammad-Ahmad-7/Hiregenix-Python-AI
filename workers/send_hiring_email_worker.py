...
import pika
import traceback
from ai_modules.send_hiring_email import send_hiring_email
from utils.constant import SEND_HIRING_EMAIL_QUEUE, SEND_HIRING_EMAIL_DELAY_QUEUE
...
setup_logging("logger/send_hiring_email_worker.log")
...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        contact_number=task['payload']['contactNumber']
        # print(email, query)
        
        result = send_hiring_email(email, query, company_email, company_name, candidate_name, logo_url, contact_number, logger)
        
        if not result:
            logger.info("Failed to send hiring email | task_id=%s", task_id)
            raise Exception("send_hiring_email function returned False")
        
        logger.info("Task %s send hiring email completed successfully", task_id)
        ...
        if retry_count >=3:
            ...
            try:
                ch.basic_publish(
                    exchange='',
                    routing_key=SEND_HIRING_EMAIL_DELAY_QUEUE,
                    body=body,
...
channel.basic_consume(
    queue=SEND_HIRING_EMAIL_QUEUE,
    on_message_callback=callback,
    auto_ack=False  # Manual ack ensures reliability
)

try:
    logger.info("Worker started and waiting for send hiring email jobs...")
