...
from utils.constant import JOB_RECOMMENDATION_QUEUE, JOB_RECOMMENDATION_DELAY_QUEUE
...
setup_logging("logger/job_recommendation_worker.log")
...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        recommendation_result=run_recommendation_pipeline(candidate_id=candidate_id)

        if not recommendation_result:
            logger.warning("Recommendation pipeline failed for candidate %s", candidate_id)
            raise Exception("Recommendation pipeline failed")
        
        logger.info("Recommendation pipeline completed successfully for candidate %s", candidate_id)
        ...
        if retry_count >=3:
            ...
            try:
                ch.basic_publish(
                    exchange='',
                    routing_key=JOB_RECOMMENDATION_DELAY_QUEUE,
                    body=body,
...
try:
    logger.info("Worker started and waiting for job recommendation jobs...")
